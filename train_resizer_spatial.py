...
def run(data_root, model_input_shape,i3d_model_path,batch_size,mode='2d', num_frames=32, data_input_shape=56,save_path='', init_lr = 0.001 ,num_epochs=10,i3d_mode='32x112', classes_file='classes.txt', freeze_i3d=True, num_resblocks=1, num_workers=4,read_at=0,  apply_at = 0, num_steps_per_update = 8):
    ...
    resizer = TransformerWithResizer(3, num_frames, (model_input_shape, model_input_shape), in_res=data_input_shape, num_resblocks=num_resblocks, read_at= read_at, apply_at=apply_at)
    if mode == 'segmented':
