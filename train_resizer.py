...
def run(data_root, model_input_shape, virat_model_path,batch_size,save_model='', init_lr = 0.001 ,num_epochs=10,v_mode='32x112', classes_file='classes.txt'):
    ...
    resizer = ResizerMainNetworkV4_2D(3, int(v_mode.split('x')[0]), model_input_shape,num_resblocks=1)
    #load the virat dataset
    train_transforms = transforms.Compose([ videotransforms.RandomHorizontalFlip(),
    ])
    dataset = Dataset(data_root, "train",classes_file,resize=True,resize_shape=(112,112), transforms=train_transforms,sample=False)
...
def main():
    ...
    batch_size = 40
    save_model = '/virat-vr/models/pytorch-i3d/resizerv42d_v2_32_112_'
