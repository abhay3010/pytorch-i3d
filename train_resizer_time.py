...
from virat_dataset import collate_tensors, collate_with_time
...
def run(data_root, model_input_shape, virat_model_path,batch_size,save_model='', init_lr = 0.001 ,num_epochs=10,v_mode='32x112', classes_file='classes.txt'):
    ...
    resizer = ResizerWithTimeCompression(3, int(v_mode.split('x')[0]), int(v_mode.split('x')[0]),  model_input_shape)
    #load the virat dataset
    train_transforms = transforms.Compose([ videotransforms.RandomHorizontalFlip(),
    ])
    dataset = Dataset(data_root, "train",classes_file,resize=False, transforms=train_transforms, num_frames=64)
...
def main():
    ...
    # virat_model_path = '/workspaces/pytorch-i3d/eval_models/v7_bilinear_16_112002800.pt'
    # batch_size = 2
    ...
    batch_size = 16
    save_model = '/virat-vr/models/pytorch-i3d/bilinear_64_32_resizer_timecompression_v2'
