...
def eval(resizer_model, model_path, root, classes_file, debug=False):
    ...
    val_dataset = Dataset(root, "test",classes_file,num_frames=32, resize=True,resize_shape=(56,56), transforms=None)
    val_dataloader = torch.utils.data.DataLoader(val_dataset, batch_size=20, shuffle=False, num_workers=5, pin_memory=True) 
