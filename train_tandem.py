...
from resizer import ResizerMainNetworkV4_2D
...
def run(root, classes_file,save_path, resizer_path, i3d_path, batch_size=256, lr=0.001):
    device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')
    ...
    helper_model = load_resizer_and_i3d(resizer_path, i3d_path)
    actual_model = load_actual_model()
...
def load_resizer_and_i3d(resizer_path, i3d_path):
    i3d = InceptionI3d(26, mode="32x112", in_channels=3)
    i3d.load_state_dict(torch.load(i3d_path))
    resizer = ResizerMainNetworkV4_2D(3, 32, (112,112),num_resblocks=1)
    resizer.load_state_dict(resizer_path)
