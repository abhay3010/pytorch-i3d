...
class InceptionI3d(nn.Module):
    def __init__(self, num_classes=157, mode='16x112', spatial_squeeze=True, name='inception_i3d', in_channels=3, dropout_keep_prob=0.5):
        ...
        if mode.endswith('224'):
            seq += [('MaxPool3d_2a_3x3', MaxPool3dSamePadding(kernel_size=[1, 3, 3], stride=(1, 2, 2), padding=0))]
        ...
        if mode.startswith('16'):
            seq += [('avg_pool', nn.AvgPool3d(kernel_size=[2, 7, 7], stride=(1, 1, 1)))]
        elif mode.startswith('32'):
            seq += [('avg_pool', nn.AvgPool3d(kernel_size=[4, 7, 7], stride=(1, 1, 1)))]
        elif mode.startswith('64'):
            seq += [('avg_pool', nn.AvgPool3d(kernel_size=[8, 7, 7], stride=(1, 1, 1)))]
        else:
            raise Exception('Unknown input temporal dinemsion')
